...
class CriticNetwork:
    ...
    def create_training_method(self):
        ...
        self.cost = tf.reduce_mean(input_tensor=tf.square(self.y_input - self.q_value_output)) + weight_decay
        self.optimizer = tf.compat.v1.train.AdamOptimizer(LEARNING_RATE).minimize(self.cost)
        self.action_gradients = tf.gradients(ys=self.q_value_output, xs=self.action_input)
